...
def parse_nums(my_string: str):
    return ''.join([x for x in my_string if x.isdigit()])

def remove_nums(my_string: str):
    return ''.join([x for x in my_string if not x.isdigit()])

# def parse_nums_and_letters(my_string: str):
#     for index, letter in enumerate(code):
#         if letter.isdigit():
#             current_num += letter
#             break
#         not_alphanumerical += letter
#         if not_alphanumerical in digits_map:
#             current_num += digits_map[not_alphanumerical]
#             not_alphanumerical = ''
...
26fmrrhhpthree6b'''
    ...
    parsed_input = [(x).strip() for x in raw_input.split('\n')]
    total = 0
    ...
    for code in parsed_input:
        ...
        for letter in code:
            ...
            not_alphanumerical += letter
            not_alphanumerical = remove_nums(not_alphanumerical)
            for key in digits_map:
                if key in not_alphanumerical:
                    current_num += digits_map[key]
                    # not_alphanumerical = ''
                    # remove key from not_alphanumerical
                    not_alphanumerical = not_alphanumerical.replace(key, '')
                    # break
        
        # print(current_num)
        total += int(f'{current_num[0]}{current_num[-1]}')
